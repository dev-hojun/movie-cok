...
(function () {
    ...
    var DraggablePiechart = function (setup) {
        ...
        this.canvas.addEventListener('touchstart', function (e) { // listener 를 달아줘야 언제 이벤트가 발생하는 지 듣고 있는 형태
            touchStart(e); // 이벤트를 담음
            e.preventDefault();
        });
        ...
        this.canvas.addEventListener('mousedown', touchStart); // 마우스 눌릴 때
        this.canvas.addEventListener('mousemove', touchMove);
        document.addEventListener('mouseup', touchEnd); // 마우스 땔 때


        this.draw();

        function touchStart(event) { // 리스너로 듣고 잇다가 이벤트가 발생하면 펑션 발동
    ...
    DraggablePiechart.prototype.defaults = {
        ...
        drawNode: function (context, piechart, x, y, centerX, centerY, hover) {
            ...
            context.save();
            context.translate(centerX, centerY);
            context.fillStyle = '#DDDDDD'; // 차트 끝의 원형 모양에 할당
